 ...
 * @subpackage Client
 */
class Kaltura_Client_Type_SegmentRanges extends Kaltura_Client_Type_BaseSegmentValue
{
	...
	public function __construct(SimpleXMLElement $xml = null, $jsonObject = null)
	{
		...
		if(!is_null($xml) && count($xml->ranges))
		{
			if(empty($xml->ranges))
				$this->ranges = array();
			else
				$this->ranges = Kaltura_Client_ParseUtils::unmarshalArray($xml->ranges, "KalturaSegmentRange");
		}
		if(!is_null($jsonObject) && isset($jsonObject->ranges))
		{
			if(empty($jsonObject->ranges))
				$this->ranges = array();
			else
				$this->ranges = Kaltura_Client_ParseUtils::jsObjectToClientObject($jsonObject->ranges, "KalturaSegmentRange");
		}
	}
	/**
	 * Range source
