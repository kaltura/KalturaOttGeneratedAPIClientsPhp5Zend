...
class Kaltura_Client_Type_AssetShopCondition extends Kaltura_Client_Type_AssetConditionBase
{
	...
	public function __construct(SimpleXMLElement $xml = null, $jsonObject = null)
	{
		...
		if(!is_null($xml) && count($xml->values) && !empty($xml->values))
			$this->values = Kaltura_Client_ParseUtils::unmarshalObject($xml->values, "KalturaStringValueArray");
		if(!is_null($jsonObject) && isset($jsonObject->values) && !empty($jsonObject->values))
			$this->values = Kaltura_Client_ParseUtils::jsObjectToClientObject($jsonObject->values, "KalturaStringValueArray");
	}
	/**
	 * Shop marker&#39;s value
	 *
