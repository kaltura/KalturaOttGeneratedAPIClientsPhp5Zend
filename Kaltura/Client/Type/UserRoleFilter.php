...
class Kaltura_Client_Type_UserRoleFilter extends Kaltura_Client_Type_Filter
{
	...
	public function __construct(SimpleXMLElement $xml = null, $jsonObject = null)
	{
		...
		if(!is_null($xml) && count($xml->currentUserRoleIdsContains))
		{
		...
		if(!is_null($jsonObject) && isset($jsonObject->currentUserRoleIdsContains))
		{
			if(!empty($jsonObject->currentUserRoleIdsContains) && ((int) $jsonObject->currentUserRoleIdsContains === 1 || strtolower((string)$jsonObject->currentUserRoleIdsContains) === 'true'))
				$this->currentUserRoleIdsContains = true;
			else
				$this->currentUserRoleIdsContains = false;
		}
		...
		if(!is_null($xml) && count($xml->profileEqual))
			$this->profileEqual = (string)$xml->profileEqual;
		if(!is_null($jsonObject) && isset($jsonObject->profileEqual))
			$this->profileEqual = (string)$jsonObject->profileEqual;
	}
	/**
