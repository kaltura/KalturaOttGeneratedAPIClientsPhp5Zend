 ...
 * @subpackage Client
 */
class Kaltura_Client_Type_AssetUserRule extends Kaltura_Client_Type_AssetRuleBase
{
	...
	public function __construct(SimpleXMLElement $xml = null, $jsonObject = null)
	{
		...
		if(!is_null($jsonObject) && isset($jsonObject->conditions))
		{
			if(empty($jsonObject->conditions))
				...
				$this->conditions = Kaltura_Client_ParseUtils::jsObjectToClientObject($jsonObject->conditions, "KalturaAssetConditionBase");
		}
		if(!is_null($xml) && count($xml->actions))
		{
			if(empty($xml->actions))
				...
				$this->actions = Kaltura_Client_ParseUtils::unmarshalArray($xml->actions, "KalturaAssetUserRuleAction");
		}
		if(!is_null($jsonObject) && isset($jsonObject->actions))
		{
			if(empty($jsonObject->actions))
				...
				$this->actions = Kaltura_Client_ParseUtils::jsObjectToClientObject($jsonObject->actions, "KalturaAssetUserRuleAction");
		}
	}
	/**
