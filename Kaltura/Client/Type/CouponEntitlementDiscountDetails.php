...
class Kaltura_Client_Type_CouponEntitlementDiscountDetails extends Kaltura_Client_Type_EntitlementDiscountDetails
{
	...
	public function __construct(SimpleXMLElement $xml = null, $jsonObject = null)
	{
		...
		if(!is_null($xml) && count($xml->endlessCoupon))
		{
		...
		if(!is_null($jsonObject) && isset($jsonObject->endlessCoupon))
		{
			if(!empty($jsonObject->endlessCoupon) && ((int) $jsonObject->endlessCoupon === 1 || strtolower((string)$jsonObject->endlessCoupon) === 'true'))
				$this->endlessCoupon = true;
			else
				$this->endlessCoupon = false;
		}
