...
class Kaltura_Client_Type_UserSessionCondition extends Kaltura_Client_Type_UserSessionProfileExpression
{
	...
	public function __construct(SimpleXMLElement $xml = null, $jsonObject = null)
	{
		...
		if(!is_null($xml) && count($xml->condition) && !empty($xml->condition))
			$this->condition = Kaltura_Client_ParseUtils::unmarshalObject($xml->condition, "KalturaCondition");
		if(!is_null($jsonObject) && isset($jsonObject->condition) && !empty($jsonObject->condition))
			$this->condition = Kaltura_Client_ParseUtils::jsObjectToClientObject($jsonObject->condition, "KalturaCondition");
	}
	/**
	 * expression
	 *
