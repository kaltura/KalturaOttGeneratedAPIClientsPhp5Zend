...
class Kaltura_Client_UserRoleService extends Kaltura_Client_ServiceBase
{
	 ...
	 */
	function add(Kaltura_Client_Type_UserRole $role)
	{
		$kparams = array();
	...
	function delete($id)
	{
		...
		$this->client->queueServiceActionCall("userrole", "delete", null, $kparams);
		if ($this->client->isMultiRequest())
			return $this->client->getMultiRequestResult();
		$resultXml = $this->client->doQueue();
		$resultXmlObject = new \SimpleXMLElement($resultXml);
	 ...
	 * @throws Kaltura_Client_Exception|Kaltura_Client_ClientException
	 */
	function update($id, Kaltura_Client_Type_UserRole $role)
	{
		$kparams = array();
		$this->client->addParam($kparams, "id", $id);
		$this->client->addParam($kparams, "role", $role->toParams());
